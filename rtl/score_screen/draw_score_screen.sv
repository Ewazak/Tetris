...
*
* Description: draw_score_screen module - displays a scaled "score screen" image from ROM.
*/
module draw_score_screen #(
    parameter SCALE = 16
)(
    input  logic clk,
    input  logic rst,
    ...
    logic [11:0] pixel_data;       // ROM output data
    logic [11:0] rgb_nxt;

    // ROM instance (small image)
    score_screen_rom rom_inst (
        .clk(clk),
        .addr(pixel_addr),
        .pixel_data(pixel_data)
    );

    // Pipeline 1 - compute ROM address with scaling
    ...
    // Pipeline 3 - delay VGA signals (2 cycles)
