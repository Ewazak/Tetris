...
/*
 ...
 Description:   Renders text string on the screen by using multiple "draw_rect_char" module.
 */
//////////////////////////////////////////////////////////////////////////////
 module draw_text #(
    parameter SCALE       = 4,
...
always_ff @(posedge clk) begin : state_reg_blk
    if (rst) begin
        state  <= LOAD;
        char_i <= 0;
        line_i <= 0;
...
always_comb begin : merge_comb_blk
    rgb_out_nxt = rgb_in;
    for (i = 0; i < LEN; i++) begin
        if (draw_rgb[i] != rgb_in) begin
            rgb_out_nxt = draw_rgb[i];
            break;
        end
    end
end
