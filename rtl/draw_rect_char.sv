//////////////////////////////////////////////////////////////////////////////
/*
 2025  AGH University of Science and Technology
 MTM UEC2
 Module name:   draw_rect_char
 Author:        Ewa Żakowska, Adrianna Solińska
 Description:   Draws a single ASCII character at a given position.
 */
//////////////////////////////////////////////////////////////////////////////
 module draw_rect_char #(
    parameter ORIGIN_X = 100,
    parameter ORIGIN_Y = 100,
    parameter CHAR_WIDTH  = 8,
    parameter CHAR_HEIGHT = 16,
    parameter SCALE = 1
)(
    input  logic clk,          // posedge active clock
    input  logic rst,          // high-level active synchronous reset
    input  logic [7:0] char_line_pixels, // 8-bit font row
    // VGA input signals
    input  logic [10:0] hcount,
    input  logic [10:0] vcount,
    input  logic        hblnk,
    input  logic        vblnk,
    input  logic [11:0] rgb_in,
    // RGB output
    output logic [11:0] rgb_out
);

//------------------------------------------------------------------------------
// local variables
//------------------------------------------------------------------------------
logic [2:0]  pixel_bit;
logic [11:0] rgb_out_nxt;

//------------------------------------------------------------------------------
// output register with sync reset
//------------------------------------------------------------------------------
always_ff @(posedge clk) begin : out_reg_blk
    if(rst) begin : out_reg_rst_blk
        rgb_out <= 12'h000;
    end
    else begin : out_reg_run_blk
        rgb_out <= rgb_out_nxt;
    end
end

//------------------------------------------------------------------------------
// logic
//------------------------------------------------------------------------------
assign pixel_bit = 7 - (((hcount - ORIGIN_X)/SCALE) % CHAR_WIDTH);

always_comb begin : out_comb_blk
    rgb_out_nxt = rgb_in;

    if (vblnk || hblnk) begin
        // No change during blanking periods
    end
    else if (vcount >= ORIGIN_Y && vcount < ORIGIN_Y + CHAR_HEIGHT*SCALE &&
             hcount >= ORIGIN_X && hcount < ORIGIN_X + CHAR_WIDTH*SCALE) begin
        if (char_line_pixels[pixel_bit])
            rgb_out_nxt = 12'hFFF;
    end
end

endmodule